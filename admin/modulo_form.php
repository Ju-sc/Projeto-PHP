...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
 ...
 if (empty($erro)) {
    ...
    if (!empty($_POST["id"])) {
        ...
        $sql2 = "SELECT * FROM modulos WHERE titulo = '$titulo'";
    ...
    if (empty($erro)) {
        if (mysqli_query($conexao, $sql)) {
            $sucesso = "Módulo salvo com sucesso!";
            header("Location: cursos.php");
            exit;
        } else {
            $erro = "Erro ao salvar módulo.";
        }
    }
    ...
    <main class="flex-1 flex flex-col">
        ...
        <div class="p-6 flex-1 max-w-xl">
            <div class="bg-white rounded-xl shadow-sm p-6">
                <form action="modulo_form.php?id=<?= $editando['id'] ?? '' ?>" method="post" enctype="multipart/form-data">
